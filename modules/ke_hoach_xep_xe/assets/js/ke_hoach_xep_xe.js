(function ($, Drupal) {
  ...
  var LIST_SNAPSHOT_CACHE_KEY = 'ke_hoach_xep_xe_list_snapshot_v4';
  ...
  function customerDisplayName(customer, useShortCode) {
    // Kế hoạch có thể mất khách hàng (bản ghi bị xoá): trả rỗng thay vì làm
    // hỏng cả danh sách.
    if (!customer) return '';
  ...
  // Thao tác đổi Đã cắt mooc <-> Đủ hàng do server trả về (hanh_dong_cont) cho người có quyền sửa.
  var PORT_CONT_TOGGLE_CONFIRM = {
    danh_dau_du_hang: { title: 'Xác nhận cont đã đủ hàng?', text: 'Cont sẽ chuyển từ "Đã cắt mooc" sang "Đủ hàng".', payload: { da_du_hang: 1 } },
    bo_du_hang: { title: 'Bỏ đánh dấu đủ hàng?', text: 'Cont sẽ quay về trạng thái "Đã cắt mooc".', payload: { da_du_hang: 0 } }
  };

  function portContToggleAction(row) {
    var found = null;
    $.each((row && row.hanh_dong_cont) || [], function (_, action) {
      if (PORT_CONT_TOGGLE_CONFIRM[action.action]) found = action;
    });
    return found;
  }

  // Thẻ trạng thái cont. Đã cắt mooc / Đủ hàng bấm được (có hộp xác nhận) khi server cho phép.
  function portContStatusBadgeHtml(row) {
    var contStatus = String((row && row.trang_thai_cont) || '');
    if (!contStatus) return '';
    var color = PORT_CONT_STATUS_COLOR[contStatus] || 'bg-label-secondary';
    var toggle = portContToggleAction(row);
    if (toggle) {
      return '<span class="badge rounded-pill khxh-cont-toggle btn-hang-cang-cont-toggle ' + color + '" role="button" tabindex="0" data-id="' + (parseInt(row.nid, 10) || 0) + '" data-cont="' + escHtml(row.so_cont || '') + '" data-action="' + escHtml(toggle.action) + '" title="Trạng thái cont: ' + escHtml(contStatus) + '. Bấm để ' + escHtml(String(toggle.label || '').toLowerCase()) + '"><i class="ti tabler-package me-1"></i>' + escHtml(contStatus) + '</span>';
    }
    return '<span class="badge rounded-pill ' + color + '" title="Trạng thái cont: ' + escHtml(contStatus) + '"><i class="ti tabler-package me-1"></i>' + escHtml(contStatus) + '</span>';
  }

  // Bấm thẻ trạng thái cont: hỏi xác nhận rồi đổi Đã cắt mooc <-> Đủ hàng và tải lại danh sách.
  function requestPortContToggle(id, action, contNumber, $badge) {
    var config = PORT_CONT_TOGGLE_CONFIRM[action];
    if (!id || !config) return;
    var run = function () {
      if ($badge && $badge.length) $badge.addClass('khxh-cont-toggle-busy');
      $.ajax({
        url: '/api/quan-ly-cont/' + id,
        type: 'PUT',
        contentType: 'application/json; charset=utf-8',
        ...
        if (res.status !== 'success') {
          if ($badge && $badge.length) $badge.removeClass('khxh-cont-toggle-busy');
          if (notyf) notyf.error(res.message || 'Cập nhật trạng thái cont thất bại');
          return;
        }
    ...
    var text = (contNumber ? 'Cont ' + contNumber + ': ' : '') + config.text;
    if (typeof Swal === 'undefined') {
      if (window.confirm(config.title)) run();
      return;
    }
    Swal.fire({
      title: config.title,
  ...
  function transportCardHtml(title, row, emptyText) {
    ...
    var routeTo = row.bai_ha_ngoai || row.bai_ha_thuc_te || row.bai_ha_cont || row.diem_den || '';
  ...
  function renderDetailModal(d) {
    ...
    var diemDen = d.bai_ha_ngoai || d.bai_ha_thuc_te || d.bai_ha_cont || '';
    ...
    var containerHtml = '' +
      ...
      '<div class="detail-route-grid">' +
        ...
        detailRoutePoint('Bãi hạ', d.bai_ha_ngoai || d.bai_ha_thuc_te || d.bai_ha_cont || '') +
  ...
  function initList() {
    ...
    $(document).on('click keydown', '.btn-hang-cang-cont-toggle', function (e) {
      if (e.type === 'keydown' && e.which !== 13 && e.which !== 32) return;
      e.preventDefault();
      e.stopPropagation();
      var $badge = $(this);
      requestPortContToggle(parseInt($badge.attr('data-id'), 10) || 0, String($badge.attr('data-action') || ''), String($badge.attr('data-cont') || ''), $badge);
    });
  ...
  // Modal xếp xe hàng cảng: theo dõi thay đổi chưa lưu để cảnh báo khi đóng.
  // Chỉ tính thao tác của người dùng (sự kiện gốc từ trình duyệt, chọn Select2,
  ...
  function isHangCangDirtyIgnoredTarget(target) {
    var $t = $(target);
    if ($t.is('[type=file], [type=search], .select2-search__field')) return true;
    var idAndClass = String($t.attr('id') || '') + ' ' + String($t.attr('class') || '');
    return /search|filter|plan-file/i.test(idAndClass);
  }

  function bindHangCangEditDirtyTracking() {
    var $modal = $('#ke-hoach-edit-fullscreen-modal');
    if (!$modal.length || $modal.data('khxhDirtyBound')) return;
    $modal.data('khxhDirtyBound', true);
    ...
    $modal.on('hide.bs.modal', function (e) {
      if (e.target !== this || !hangCangEditDirty || hangCangEditCloseConfirmed) return;
      e.preventDefault();
      e.stopImmediatePropagation();
      var modalEl = this;
      var closeAnyway = function () {
        hangCangEditCloseConfirmed = true;
        resetHangCangEditDirty();
        var instance = bootstrap.Modal.getInstance(modalEl);
        if (instance) instance.hide();
        hangCangEditCloseConfirmed = false;
      };
      if (typeof Swal !== 'undefined') {
        Swal.fire({
          title: 'Thoát mà chưa lưu?',
          text: 'Các thay đổi trong kế hoạch này sẽ bị mất.',
          icon: 'warning',
          showCancelButton: true,
          ...
          if (result.isConfirmed) closeAnyway();
        });
      } else if (window.confirm('Thoát mà chưa lưu? Các thay đổi sẽ bị mất.')) {
        closeAnyway();
      }
    });
  }

  // ---- Dòng danh sách: mỗi màn một hàm dựng riêng, không dùng chung nhánh. ----

  function rowActionsTriggerHtml(stt) {
    return '<td><button type="button" class="khxh-row-actions-trigger" title="Mở chức năng kế hoạch" aria-label="Mở chức năng kế hoạch #' + stt + '">' + stt + '</button></td>';
  }

  function listHinhThucBadgeHtml(row) {
    return row.hinh_thuc_van_tai ? '<span class="badge ' + hinhThucColor(row.hinh_thuc_van_tai) + '" title="Hình thức vận tải: ' + escHtml(hinhThucLabel(row.hinh_thuc_van_tai)) + '">' + escHtml(hinhThucLabel(row.hinh_thuc_van_tai)) + '</span>' : '';
  }

  // Hàng cảng (/ke-hoach-xep-xe).
  function hangCangListRowHtml(row, stt) {
    ...
    var baiHaDisplay = row.bai_ha_ngoai || row.bai_ha_cont || '';
    var customerTitle = 'Khách hàng: ' + (khName ? (khName + (khFullName ? ' - ' + khFullName : '')) : 'Chưa có') + (row.so_bkg ? '\nBKG: ' + row.so_bkg : '');
    var customerDisplay = khName ? escHtml(khName) : '_';
    if (row.so_bkg) {
    ...
    return '<tr>' +
      ...
      '<td class="khxh-common-cell">' +
        ...
        '</div>' +
      '</td>' +
      '<td class="khxh-container-cell" title="' + escHtml(containerTitle) + '">' +
      ...
      '<td class="khxh-kho-cell" title="Địa chỉ kho: ' + escHtml(row.dia_chi_kho || 'Chưa có') + '">' + (row.dia_chi_kho ? escHtml(row.dia_chi_kho) : '_') + '</td>' +
      '<td class="text-nowrap">' +
        '<div class="khxh-hanh-trinh-cell" title="' + escHtml(hanhTrinhTitle) + '">' +
          '<div class="khxh-hanh-trinh-box">' + (baiLayDisplay ? escHtml(baiLayDisplay) : '_') + '</div><div class="khxh-hanh-trinh-separator"></div><div class="khxh-hanh-trinh-box">' + (baiHaDisplay ? escHtml(baiHaDisplay) : '_') + '</div>' +
        '</div>' +
      '</td>' +
  ...
  function tuyenXaListRowHtml(row, stt) {
    ...
    var customerDisplay = khName ? escHtml(khName) : '<span class="text-muted fst-italic small">khách hàng</span>';
    var rowActionMenu = '<span class="khxh-row-action-menu">' + buildActions(row) + '</span>';
    return '<tr>' +
      rowActionsTriggerHtml(stt) +
      '<td class="khxh-date-cell"><div class="khxh-date-stack">' + (dateOnlyStack(row.created) || '<span class="text-muted">—</span>') + (hinhThucStatus ? '<br><span class="khxh-htvt-status ' + planRoleClass + '">' + escHtml(hinhThucStatus) + '</span>' : '') + '</div>' + rowActionMenu + '</td>' +
      '<td class="khxh-common-cell">' +
        '<div class="khxh-customer-cell"' + (khFullName ? ' title="' + escHtml(khFullName) + '"' : '') + '>' + customerDisplay + '</div>' +
        '<div class="khxh-htvt-cell">' +
          ((hinhThucBadge || returnContText) ? '<div class="khxh-htvt-badge-wrap">' + hinhThucBadge + (returnContText ? '<span class="khxh-return-cont-list" title="' + escHtml('Cont kéo về: ' + returnContText) + '">' + escHtml(returnContText) + '</span>' : '') + '</div>' : '') +
        '</div>' +
      '</td>' +
      ...
      '</td>' +
      '<td class="khxh-vehicle-cell">' + vehicleListInfoHtml(row) + '</td>' +
      '<td class="khxh-kho-cell">' + (row.dia_chi_kho ? escHtml(row.dia_chi_kho) : '') + '</td>' +
      '<td class="text-nowrap">' +
  ...
  function loadList() {
    ...
    params.limit = 50;
    // Huỷ request danh sách trước đó để response đến trễ không ghi đè kết quả mới.
    ...
    var currentXhr = listXhr = $.ajax({
      ...
      success: function (res) {
        ...
        var renderRow = currentPlanType() === 'tuyen_xa' ? tuyenXaListRowHtml : hangCangListRowHtml;
        var html = '';
        for (var i = 0; i < items.length; i++) {
          html += renderRow(items[i], (resp.current_page - 1) * pageSize + i + 1);
        }
  ...
  function initForm() {
    ...
    $(document)
      ...
      .off('change', '.line-bai-ha-ngoai-toggle')
      ...
      .off('change', '.line-cont-picker-wrap .line-cont-bai-ha-ngoai-select')
    ...
    function initCardUi($card, line) {
      ...
      initSelect2($card.find('.line-bai-ha-ngoai-select')[0], '— Chọn bãi hạ ngoài —');
      attachCreateOption($card.find('.line-bai-ha-ngoai-select'), 'Bãi', line, 'bai_ha_ngoai');
    ...
    // Bãi hạ ngoài (hàng cảng): cont hạ ở bãi ngoài thay vì bãi hạ theo booking (vd. kiểm dịch).
    // Lưu vào bai_ha_ngoai (varchar; chặng định mức lái xe nhận theo bãi này). Không có cờ bật/tắt riêng trong DB:
    // có giá trị = kế hoạch có hạ bãi ngoài. Tắt công tắc rồi LƯU thì giá trị bị xoá; chưa lưu thì bật/tắt chỉ ẩn/hiện
    // ô và giữ nguyên giá trị đang chọn. Trạng thái chưa lưu nằm ở line.bai_ha_ngoai_enabled / line.bai_ha_ngoai_value.
    function portBaiHaNgoaiEnabled(line) {
      if (line && typeof line.bai_ha_ngoai_enabled !== 'undefined') return !!parseInt(line.bai_ha_ngoai_enabled, 10);
      return !!String((line && line.bai_ha_ngoai) || '').trim();
    }

    function portBaiHaNgoaiShownValue(line) {
      if (line && typeof line.bai_ha_ngoai_value !== 'undefined') return line.bai_ha_ngoai_value;
      return (line && line.bai_ha_ngoai) || '';
    }

    function portBaiHaNgoaiSwitchHtml(line) {
      return '<div class="khxh-hang-cang-switches"><label class="form-check form-switch mb-0"><input class="form-check-input line-bai-ha-ngoai-toggle" type="checkbox"' + (portBaiHaNgoaiEnabled(line) ? ' checked' : '') + '><span class="form-check-label">Bãi hạ ngoài</span></label></div>';
    }

    // Nút tuyến vận chuyển "Bãi hạ ngoài" nằm giữa Địa chỉ kho và Bãi hạ (theo booking).
    function portBaiHaNgoaiNodeHtml(line) {
      var hidden = portBaiHaNgoaiEnabled(line) ? '' : ' d-none';
      return '<div class="khxh-route-node line-bai-ha-ngoai-fields' + hidden + '"><label class="form-label">Bãi hạ ngoài</label><select class="form-select line-bai-ha-ngoai-select">' + buildTagOptions(state.diaDiem.bai, portBaiHaNgoaiShownValue(line)) + '</select></div>' +
        '<div class="khxh-route-arrow line-bai-ha-ngoai-fields' + hidden + '"><i class="ti tabler-arrow-right"></i></div>';
    }

    // Đọc Bãi hạ ngoài từ card vào line.
    // - bai_ha_ngoai_value: giá trị đang chọn, giữ cả khi công tắc tắt (chưa lưu) để bật lại không mất.
    // - bai_ha_ngoai: giá trị SẼ LƯU = giá trị đang chọn nếu công tắc bật, rỗng nếu tắt (lưu là xoá).
    // Bỏ giá trị trùng bãi hạ theo booking.
    function syncPortBaiHaNgoai($row, line) {
      var enabled = $row.find('.line-bai-ha-ngoai-toggle').is(':checked');
      var value = normalizeBaiHaThucTe(($row.find('.line-bai-ha-ngoai-select').val() || '').trim(), line.bai_ha_cont);
      line.bai_ha_ngoai_enabled = enabled ? 1 : 0;
      line.bai_ha_ngoai_value = value;
      line.bai_ha_ngoai = enabled ? value : '';
    }
        ...
        html += '' +
          '<div class="ke-hoach-line-card ' + modalClassPrefix + '-card khxh-main-plan-card" id="khxh-main-plan-' + escHtml(line.key) + '" data-line-key="' + line.key + '">' +
            '<div class="' + modalClassPrefix + '-card-head">' +
              ...
              '</div>' : portBaiHaNgoaiSwitchHtml(line)) +
            '</div>' +
            ...
            '<div class="' + modalClassPrefix + '-section">' +
              ...
              '<div class="khxh-route-flow">' +
                ...
                (isTuyenXa ? '' : portBaiHaNgoaiNodeHtml(line)) +
                '<div class="khxh-route-node"><label class="form-label">Bãi hạ' + (isTuyenXa ? ' <span class="text-danger">*</span>' : ' (theo booking)') + '</label><select class="form-select line-bai-ha-select"' + (executionPlan || mainWorkDone ? ' disabled' : '') + '>' + buildTagOptions(state.diaDiem.bai, line.bai_ha_cont) + '</select></div>' +
    ...
    function syncLine($row) {
      ...
      if (!useTableLayout) {
        ...
        } else if ($row.find('.line-bai-ha-ngoai-toggle').length) {
          // Hàng cảng: Bãi hạ ngoài theo công tắc riêng; tắt thì xoá giá trị khi lưu.
          // Bãi lấy thực tế giữ nguyên (không bị xoá theo).
          syncPortBaiHaNgoai($row, line);
        } else {
    ...
    function contRefButtonText(line) {
      ...
      if (currentPlanType() !== 'tuyen_xa') {
        ...
        var baiHa = item.bai_ha_ngoai || item.bai_ha_cont || item.bai_ha || '';
    ...
    function normalContReturnRouteText(item) {
      ...
      var actualDestination = normalizeBaiHaThucTe(item.bai_ha_ngoai || '', plannedDestination);
    ...
    function renderContCandidateRows(line, $card) {
      ...
      for (var c = 0; c < candidates.length; c++) {
        ...
        var actualBaiHa = normalizeBaiHaThucTe(item.bai_ha_ngoai || '', plannedBaiHa);
        ...
        rows.push('<div class="cont-picker-row' + (currentPlanType() === 'tuyen_xa' ? ' is-tuyen-xa' : '') + (selected ? ' is-selected' : '') + (canSelect ? '' : ' is-disabled') + '" title="' + escHtml(canSelect ? generalTooltip : candidates[c].disabledReason) + '">' +
          ...
          (currentPlanType() === 'tuyen_xa'
            ...
            : '<div class="cont-picker-cell cont-picker-destination-cell">' +
                ...
                '</label>' +
                '<select class="form-select form-select-sm line-cont-bai-ha-ngoai-select" data-id="' + item.nid + '" data-planned="' + escHtml(plannedBaiHa) + '">' + buildTagOptions(state.diaDiem.bai, destinationValue) + '</select>' +
              '</div>') +
      ...
      $body.html(rows.length ? rows.join('') : '<div class="text-center text-muted py-4">Không có cont phù hợp</div>');
	      ...
	      $body.find('.line-cont-bai-ha-ngoai-select').each(function () {
    ...
    // Modal chọn cont kéo về: "Hạ theo booking" = rỗng, chọn bãi khác = bãi hạ ngoài (bai_ha_ngoai).
    function stageContBaiHaNgoai($card, contId, value) {
      contId = parseInt(contId, 10) || 0;
      if (!contId) return;
      state.pendingContDestinationUpdates[String(contId)] = value || '';
      updateContCandidateItem($card, contId, { bai_ha_ngoai: value || '' });
      var $picker = getContPicker($card);
      var $select = $picker.find('.line-cont-bai-ha-ngoai-select[data-id="' + contId + '"]');
    ...
    function flushPendingContDestinationUpdates() {
      ...
      var requests = $.map(ids, function (id) {
        ...
        $.ajax({
          ...
          data: JSON.stringify({ bai_ha_ngoai: updates[id] || '' }),
          dataType: 'json'
        }).done(function (res) {
          if (res && res.status === 'success') {
            deferred.resolve(res);
          } else {
            deferred.reject({ responseText: JSON.stringify(res || { message: 'Không cập nhật được bãi hạ ngoài' }) });
          }
        }).fail(function (jqXHR) {
    ...
    // Xoá trạng thái lỗi cũ của một dòng trước khi kiểm tra lại (dùng chung, chỉ là UI).
    function clearLineValidationUi($row) {
      if (useTableLayout) {
        $row.removeClass('table-danger');
        $row.find('.line-customer-feedback').hide();
    ...
    function lineValidationRows() {
      return $form(useTableLayout ? '#ke-hoach-lines-body .ke-hoach-table-row' : '#ke-hoach-lines .ke-hoach-line-card');
    }

    function validateForm() {
      return currentPlanType() === 'tuyen_xa' ? validateTuyenXaForm() : validatePortForm();
    }
    ...
    function validatePortForm() {
      ...
      lineValidationRows().each(function () {
        var $row = $(this);
        var line = syncLine($row);
        clearLineValidationUi($row);
        ...
        if (!line.dia_chi_kho) {
          ok = false;
          if (!useTableLayout) $row.addClass('line-card-invalid');
    ...
    function validateTuyenXaForm() {
      ...
      lineValidationRows().each(function () {
        ...
        if (isExecutionPlan(line) && !line.ke_hoach_cont_ref_nid) {
          ok = false;
          $row.addClass('line-card-invalid');
        ...
        if (!isExecutionPlan(line) && !line.hinh_thuc_van_tai) {
          ok = false;
          $row.addClass('line-card-invalid');
        ...
        if (isExecutionPlan(line) && line.ke_hoach_cont_ref_nid && (parseInt(line.cont_thuc_hien_tu_index, 10) < 0 || parseInt(line.cont_thuc_hien_den_index, 10) <= parseInt(line.cont_thuc_hien_tu_index, 10))) {
          ok = false;
          $row.addClass('line-card-invalid');
        ...
        if (line.bai_ha_tam_1_enabled && !line.bai_ha_tam_1) {
          ok = false;
          $row.addClass('line-card-invalid');
          setSelect2Invalid($row.find('.line-bai-ha-tam-1-select'), true);
        }
        if (line.bai_ha_tam_2_enabled && !line.bai_ha_tam_2) {
          ok = false;
          $row.addClass('line-card-invalid');
    ...
    // Payload lưu kế hoạch. Phần chung của một dòng nằm ở linePayloadCommon();
    // thong_tin_json do hàm riêng của từng màn dựng.
    function portLineJson(line) {
      return { kiem_dich: line.kiem_dich || 0, kiem_hoa: line.kiem_hoa || 0, hun_trung: line.hun_trung || 0, cont_keo_ve_seal_phu: line.cont_keo_ve_seal_phu || 0, cont_keo_ve_kiem_dich: line.cont_keo_ve_kiem_dich || 0, cont_keo_ve_kiem_hoa: line.cont_keo_ve_kiem_hoa || 0, cont_keo_ve_hun_trung: line.cont_keo_ve_hun_trung || 0 };
    }

    function tuyenXaLineJson(line) {
      return { vai_tro_ke_hoach: planRole(line), cong_viec_chinh_hoan_thanh: line.cong_viec_chinh_hoan_thanh || 0, hinh_thuc_tinh_luong_lai_xe: line.hinh_thuc_tinh_luong_lai_xe || 'khoan', ke_hoach_ket_hop_enabled: line.ke_hoach_ket_hop_enabled || 0, bai_ha_tam_1_enabled: line.bai_ha_tam_1_enabled || 0, bai_ha_tam_1: line.bai_ha_tam_1 || '', bai_ha_tam_2_enabled: line.bai_ha_tam_2_enabled || 0, bai_ha_tam_2: line.bai_ha_tam_2 || '', vi_tri_cont_hien_tai: line.vi_tri_cont_hien_tai || '', vi_tri_cont_index_hien_tai: parseInt(line.vi_tri_cont_index_hien_tai, 10) || 0, cont_thuc_hien_tu_index: parseInt(line.cont_thuc_hien_tu_index, 10), cont_thuc_hien_den_index: parseInt(line.cont_thuc_hien_den_index, 10), cont_thuc_hien_chang: line.cont_thuc_hien_chang || [], cont_keo_ve_tu: line.cont_keo_ve_tu || '', cont_keo_ve_den: line.cont_keo_ve_den || '' };
    }

    function linePayloadCommon(line) {
      return {
        ...
        bai_ha_ngoai: line.bai_ha_ngoai || '',
    ...
    // Hàng cảng không dùng bãi lấy/hạ thực tế (bai_lay_thuc_te, bai_ha_thuc_te): không gửi lên,
    // bãi hạ ngoài đi bằng bai_ha_ngoai. Tuyến xa giữ nguyên phần chung.
    function portLinePayloadCommon(line) {
      var payload = linePayloadCommon(line);
      delete payload.bai_lay_thuc_te;
      delete payload.bai_ha_thuc_te;
      return payload;
    }
    ...
    function gatherCreatePayload() {
      ...
      var lineJson = currentPlanType() === 'tuyen_xa' ? tuyenXaLineJson : portLineJson;
      var linePayload = currentPlanType() === 'tuyen_xa' ? linePayloadCommon : portLinePayloadCommon;
      return {
    ...
    function updateCompleteButton(row) {
      if (currentPlanType() === 'tuyen_xa') updateTuyenXaStatusButton(row);
      else updatePortStatusButton(row);
    }

    // Nút trạng thái hàng cảng (#complete-plan-btn): màu theo hangCangPlanStatusColor().
    function updatePortStatusButton(row) {
    ...
    function updateTuyenXaStatusButton(row) {
      var $btn = $form('#khxh-status-btn');
      if (!$btn.length) return;
      var nid = row && row.nid ? parseInt(row.nid, 10) : parseInt($form('#nid-input').val(), 10);
      if (!nid) {
        $btn.addClass('d-none').removeAttr('data-id');
    ...
    // Công tắc Bãi hạ ngoài (hàng cảng): chỉ ẩn/hiện ô, KHÔNG xoá giá trị (chỉ xoá khi lưu với công tắc tắt).
    $(document).on('change', '.line-bai-ha-ngoai-toggle', function () {
      var $card = $(this).closest('.ke-hoach-line-card');
      $card.find('.line-bai-ha-ngoai-fields').toggleClass('d-none', !this.checked);
      syncLine($card);
    });
    ...
    if (planType === 'tuyen_xa') {
      // Tuyến xa giữ nguyên hành vi cũ.
      ...
      // Hàng cảng: Enter chỉ lưu khi đang ở ô nhập thường. Trong ô tìm của
      // Select2, ô chọn ngày, nút bấm hay danh sách chọn thì Enter dành cho
      // thao tác của chính ô đó, tránh lưu nhầm cả kế hoạch.
      $form('#ke-hoach-form').on('keydown', function (e) {
        if (e.which !== 13) return;
        var $target = $(e.target);
        if ($target.is('textarea, button, a, select, [type=button], [type=submit], [type=file], .select2-search__field, .flatpickr-input, .flatpickr-date')) return;
        if ($target.closest('.select2-container, .flatpickr-calendar').length) return;
        e.preventDefault();
        $form('#save-btn').trigger('click');
      });
    }
    ...
    $(document).on('change', '.line-bai-ha-theo-ke-hoach-checkbox', function () {
      ...
      if ($checkbox.is(':checked')) {
        stageContBaiHaNgoai($card, contId, '');
        return;
      }
      var $select = $checkbox.closest('.cont-picker-destination-cell').find('.line-cont-bai-ha-ngoai-select');
      if ($select.data('select2')) $select.select2('open');
      else $select.focus();
    });
    $(document).on('change', '.line-cont-picker-wrap .line-cont-bai-ha-ngoai-select', function () {
	      var $select = $(this);
      ...
      if (!contId) return;
      stageContBaiHaNgoai($card, contId, normalizeBaiHaThucTe(value, planned));
    });
        ...
        for (var i = 0; i < items.length; i++) {
          ...
          var daDuHang = parseInt(item.da_du_hang, 10) === 1;
            html += '<tr data-id="' + item.nid + '">' +
              ...
              '<td>' + escHtml(item.bai_ha_ngoai || '') + '</td>' +
