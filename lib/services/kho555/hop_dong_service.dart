...
class HopDongService {
  static const String _endpoint = AuthService.hopDongEndpoint;

  static Future<Map<String, dynamic>> fetchHopDong({
    int page = 1,
    int limit = 20,
    String keyword = '',
  }) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (items != null) {
          final list = items
              .whereType<Map>()
              .map((e) => HopDong.fromJson(Map<String, dynamic>.from(e)))
              .toList();
          return {'items': list, 'pagination': pagination};
        }

        throw Exception('Không tìm thấy dữ liệu hợp đồng');
      }
  ...
  static Future<ApiResponse> saveHopDong(Map<String, dynamic> data) async {
    return _request(
      url: _endpoint,
      method: 'POST',
      data: data,
      fallbackMessage: 'Không thể lưu hợp đồng',
    );
  }

  static Future<ApiResponse> updateHopDong(int nid, Map<String, dynamic> data) async {
    return _request(
      url: '$_endpoint/$nid',
      method: 'PUT',
      data: data,
  ...
  static Future<ApiResponse> deleteHopDong(int nid) async {
    return _request(
      url: '$_endpoint/$nid',
      method: 'DELETE',
      data: {},
      fallbackMessage: 'Không thể xoá hợp đồng',
    );
  }
  ...
  static Future<ApiResponse> _request({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        message = res['message']?.toString() ?? 'Lỗi server: ${response.statusCode}';
      }
  ...
  static List<dynamic> _decodeContentList(dynamic content) {
    ...
    if (content is Map) return _extractList(content);
    throw Exception('Dữ liệu hợp đồng không hợp lệ từ Worker');
  }
