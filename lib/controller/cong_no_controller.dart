...
import 'package:ttk_logistics/widgets/dialog_thu_tien_cong_no.dart';
...
class CongNoController extends MyController {
  ...
  Future<void> fetchCongNo({
    ...
    try {
      if (export) {
        isExporting = true;   // 🔥 chỉ loading nút
      } else {
        isLoading = true;     // loading bảng
      }
      update();

      debugPrint(
        '➡️ AuthService.getCongNoKhachHangByChuyenXe: ${AuthService.getCongNoKhachHangByChuyenXe}',
      );
      /// ===============================
      /// BUILD PARAMS (BỔ SUNG FILTER)
      /// ===============================
      final Map<String, dynamic> params = {
        ...
        "token": token,
        "created_email": email,
        "export": export, // 🔥 GỬI SANG API
      };
      if (keywordKhachHang != null && keywordKhachHang.trim().isNotEmpty) {
        params["keyword"] = keywordKhachHang.trim();
      }

      if (trangThaiCongNo != null &&
          trangThaiCongNo.isNotEmpty &&
          trangThaiCongNo != "all") {
        params["trang_thai"] = trangThaiCongNo; // done | pending
      }
      final response = await http.post(
        Uri.parse(AuthService.workerUrl),
        headers: {"Content-Type": "application/json"},
        body: jsonEncode({
          "url": AuthService.getCongNoKhachHangByChuyenXe,
          "method": "POST",
          "params": params,
        }),
      );
      ...
      /// 🔥 NẾU LÀ EXPORT → MỞ LINK TẢI FILE
      if (export == true && res["content"]["export_file"] != null) {
        final url = res["content"]["export_file"];
...
// print('AuthService.getChuyenXeChuaThanhToanHet ${AuthService.getChuyenXeChuaThanhToanHet}'); // TODO: remove debug
...
// print('khachHangNid $khachHangNid'); // TODO: remove debug
    final response = await http.post(
      ...
      headers: {"Content-Type": "application/json"},
      body: jsonEncode({
        "url": AuthService.getChuyenXeChuaThanhToanHet,
        "method": "POST",
        "params": {
...
// print('currentUserUid $currentUserUid'); // TODO: remove debug
  ...
  Future<void> submitThuTienCongNo() async {
    ...
    try {
      ...
      debugPrint("➡️ API: ${AuthService.luuPhieuThuCongNoKhachHang}");
      ...
      final response = await http.post(
        ...
        headers: {"Content-Type": "application/json"},
        body: jsonEncode({
          "url": AuthService.luuPhieuThuCongNoKhachHang,
          "method": "POST",
          "params": payload,
        }),
      );
...
// print('response.body xem lich su thanh toan ${response.body}'); // TODO: remove debug
  ...
  Future<void> exportBangKeCongNo() async {
    // gọi API export
    // trả về file Excel / CSV / PDF
    // Web: mở link download
    // Mobile: lưu file
  }
