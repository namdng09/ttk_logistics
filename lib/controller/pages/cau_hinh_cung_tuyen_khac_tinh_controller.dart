...
class CauHinhCungTuyenKhacTinhController extends GetxController {
  ...
  /// Thông tin ngữ cảnh (mặc định hoặc khách hàng)
  ...
  Future<void> loadConfig({String? type, int? nid}) async {
    try {
      ...
      // 🔹 Xác định API theo ngữ cảnh
      final isCustomer = type == "khach_hang" && nid != null;
      final isNhaXe = type == "nha_xe" && nid != null;

      String apiUrl = AuthService.getCauHinhCungTuyen;
      Map<String, dynamic> params = {};

      if(isCustomer){
        apiUrl = AuthService.getCauHinhCungTuyenKhacTinhKhachHang;
        params = {"nid": nid};
      } else if(isNhaXe) {
        apiUrl = AuthService.getCauHinhCungTuyenKhacTinhNhaXe;
        params = {"nid": nid};
      }
      ...
      if (response.statusCode == 200) {
        ...
        if (res["success"] == true && content is List) {
          ...
          data.clear();
          _showSnack("Thất bại",
              res["content"]?.toString() ?? "Không thể tải dữ liệu", false);
        }
      } else {
        _showSnack("Lỗi mạng", "Server trả về ${response.statusCode}", false);
        await _loadOfflineData(); // fallback khi test
      }
    } catch (e) {
  ...
  Future<void> saveConfig() async {
    try {
      ...
      // 🔹 Xác định API theo ngữ cảnh
      String apiUrl =  AuthService.updateCauHinhCungTuyen;

      final isCustomer = contextType == "khach_hang" && khachHangNid != null;
      final isNhaXe = contextType == "nha_xe" && khachHangNid != null;

      final Map<String, dynamic> params = (isCustomer || isNhaXe)
          ? {"nid": khachHangNid, "content": jsonEncode(data)}
          : {"content": jsonEncode(data)};

      if(isCustomer){
        apiUrl = AuthService.updateCauHinhCungTuyenKhacTinhKhachHang;
      } else if(isNhaXe) {
        apiUrl = AuthService.updateCauHinhCungTuyenKhacTinhNhaXe;
      }
      ...
      if (response.statusCode == 200) {
        final res = jsonDecode(response.body);
        final msg = res["content"]?.toString() ?? "Không có phản hồi từ máy chủ";
  ...
  Future<void> _loadOfflineData() async {
    try {
      ...
      _showSnack("Dữ liệu mẫu", "Hiển thị dữ liệu từ file offline", true);
    } catch (e) {
