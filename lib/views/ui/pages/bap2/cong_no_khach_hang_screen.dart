...
class CongNoKhachHangScreen extends StatefulWidget {
  const CongNoKhachHangScreen({super.key});

  @override
  State<CongNoKhachHangScreen> createState() => _CongNoKhachHangScreenState();
}

class _CongNoKhachHangScreenState extends State<CongNoKhachHangScreen> with UIMixin {
  late CongNoController controller;
  ...
  void initState() {
    super.initState();
    controller = Get.put(CongNoController());
              ...
              onSelected: (v) async {
                switch (v) {
                  ...
                  // 🔥 GỌI FLOW THU TIỀN
                    ...
                    break;
                  case "update_han":
                    await controller.openUpdateHanCongNoDialog(
                      context: context,
                      chuyenXeNid: int.tryParse(item["nid"]?.toString() ?? "0") ?? 0,
                      currentHanCongNo: item["thoi_han_cong_no"],
                    );
                    break;
                }
              },
  ...
  Widget build(BuildContext context) {
    return GetBuilder<CongNoController>(
      init: controller,
      builder: (controller) {
        return Layout(
          subScreenName: 'Công nợ khách hàng',
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: DataTable(
                        ...
                        columns: const [
                          ...
                          DataColumn(label: Text("Hạn công nợ")),
